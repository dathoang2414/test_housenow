...
export const friendshipRequestRouter = router({
  send: procedure
    ...
    .mutation(async ({ ctx, input }) => {
       ...
       */
      const foundFriendship = await ctx.db
        .selectFrom('friendships')
        .select('friendships.id')
      ...
      if (foundFriendship) {
        return await ctx.db
          ...
          .set({ status: FriendshipStatusSchema.Enum.requested })
          .where('friendUserId', '=', input.friendUserId)
          .where('userId', '=', ctx.session.userId)
          .execute()
      }
  ...
  accept: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      await ctx.db.transaction().execute(async (t) => {
        ...
        await t
          .updateTable('friendships')
          .set({ status: FriendshipStatusSchema.Enum.accepted })
          .where('friendUserId', '=', userId)
          .where('userId', '=', friendUserId)
          .execute()

        const foundFriendship = await t
          .selectFrom('friendships')
          .select('friendships.id')
        ...
        if (foundFriendship) {
          ...
          await t
            ...
            .values({
              ...
              status: FriendshipStatusSchema.Enum.accepted,
            })
            .execute()
        }
      })
